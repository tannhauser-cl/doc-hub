...
function uninstall() {
  ...
  // 4. Clear the confirmation property so accidental re-run is safe
  ...
  try {
    ...
    // If audit log fails, that's OK — we're uninstalling
  }
  ...
  Logger.log('');
  Logger.log('To reinstall: run reinstall() from reinstall.gs');
}
